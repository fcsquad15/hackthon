...
const filtrarMentorTema = async (req, res) => {
  ...
  if (!habilidade) {
    return res
      .status(404)
      .json({ mensagem: "É necessário informar a habilidade" });
  }

  try {
    const mentores = await conexao.query(
      "SELECT usuarios.id,usuarios.nome,usuarios.bio, usuarios.avatar FROM agenda LEFT JOIN usuarios ON agenda.usuario_id = usuarios.id LEFT JOIN habilidadeusuarios ON usuarios.id = habilidadeusuarios.usuario_id WHERE habilidadeusuarios.habilidade_id =$1 GROUP BY usuarios.id ",
      [habilidade]
    );
    ...
    if (mentores.rowCount === 0) {
      return res
        .status(400)
        .json("Nenhum mentor encontrado para a habilidade desejada");
    }
...
const filtrarMentorArea = async (req, res) => {
  ...
  if (!area) {
    return res
      .status(404)
      .json({ mensagem: "É necessário informar a área desejada." });
  }

  try {
    const mentores = await conexao.query(
      "SELECT usuarios.id,usuarios.nome,usuarios.bio, usuarios.avatar FROM agenda LEFT JOIN usuarios ON agenda.usuario_id = usuarios.id LEFT JOIN areausuarios ON usuarios.id = areausuarios.usuario_id WHERE areausuarios.area_id =$1 GROUP BY usuarios.id",
      [area]
    );
    ...
    if (mentores.rowCount === 0) {
      return res
        .status(400)
        .json("Nenhum mentor encontrado para a área desejada.");
    }
...
const listarDias = async (req, res) => {
  ...
  if (!mentor) {
    return res
      .status(404)
      .json({ mensagem: "É necessário informar o id do mentor" });
  }
  ...
  try {
    ...
    const dias = await conexao.query(
      "SELECT dia FROM agenda WHERE usuario_id =$1 GROUP BY dia",
      [mentor]
    );
...
const listarDiasEHora = async (req, res) => {
  ...
  try {
    ...
    const dias = await conexao.query(
      "SELECT agenda.id, agenda.dia, horarios.hora FROM agenda LEFT JOIN horarios ON horarios.id=agenda.hora_id WHERE usuario_id =$1 ORDER BY agenda.dia, horarios.hora",
      [mentor]
    );
...
const listarHorarios = async (req, res) => {
  ...
  if (!mentor || !dia) {
    return res
      .status(404)
      .json({ mensagem: "É necessário informar o id do mentor e o dia" });
  }
  ...
  try {
    ...
    const horarios = await conexao.query(
      "SELECT  agenda.id AS agenda_id, horarios.id AS horario_id,horarios.hora FROM agenda LEFT JOIN horarios ON horarios.id=agenda.hora_id WHERE agenda.usuario_id =$1 AND agenda.dia=$2 ORDER BY horarios.hora",
      [mentor, dia]
    );
...
const marcarMentoria = async (req, res) => {
  ...
  if (!usuario_id || !agenda_id) {
    return res
      .status(400)
      .json({ mensagem: "Usuário ou Agenda id não informado" });
  }

  try {
    const mentoriaDisponivel = await conexao.query(
      "SELECT * FROM agenda WHERE id=$1 AND disponivel=true",
      [agenda_id]
    );
    ...
    const novaMentoria = await conexao.query(
      "INSERT INTO mentorias (usuario_mentorado_id,agenda_id) VALUES ($1,$2)",
    ...
    if (novaMentoria.rowCount === 0) {
      return res
        .status(400)
        .json({ mensagem: "Não foi possível agendar sua mentoria" });
    }

    const mentoriaMarcada = await conexao.query(
      "UPDATE agenda SET disponivel=false WHERE id=$1",
      [agenda_id]
    );
    ...
    const { rows: buscarMentorado } = await conexao.query(
      "SELECT nome FROM usuarios WHERE id = $1",
      [usuario_id]
    );

    const { rows: buscarMentor } = await conexao.query(
      "SELECT usuarios.id, usuarios.nome,agenda.dia ,horarios.hora FROM agenda LEFT JOIN usuarios ON agenda.usuario_id=usuarios.id  LEFT JOIN horarios ON agenda.hora_id=horarios.id WHERE agenda.id = $1",
      [agenda_id]
    );
    ...
    const notificaoMentorado = await conexao.query(
      "INSERT INTO notificacao (usuario_id,mensagem) VALUES ($1,$2)",
      [usuario_id, mensagemMentorado]
    );
    ...
    const notificaoMentor = await conexao.query(
      "INSERT INTO notificacao (usuario_id,mensagem) VALUES ($1,$2)",
      [buscarMentor[0].id, mensagemMentor]
    );
...
const listarMentoriasMarcadas = async (req, res) => {
  ...
  try {
    const mentorias = await conexao.query(
      "SELECT mentorias.id,agenda.dia,horarios.hora FROM mentorias    LEFT JOIN agenda ON mentorias.agenda_id=agenda.id    LEFT JOIN horarios ON agenda.hora_id=horarios.id    WHERE mentorias.usuario_mentorado_id=$1 ORDER BY agenda.dia,horarios.hora",
      [usuario_id]
    );
...
const obterMentor = async (req, res) => {
  ...
  try {
    const usuario = await conexao.query(
      "SELECT id,nome,email,bio,avatar FROM usuarios WHERE id = $1",
      [mentor]
    );
    ...
    const dias = await conexao.query(
      "SELECT agenda.id, agenda.dia, horarios.hora FROM agenda LEFT JOIN horarios ON horarios.id=agenda.hora_id WHERE usuario_id =$1 AND agenda.disponivel=true ORDER BY agenda.dia, horarios.hora",
      [mentor]
    );

    res.status(200).json({
      mentor: usuario.rows[0],
      habilidade: habilidadesUsuario.rows,
      horarios: dias.rows,
    });
  } catch (error) {
